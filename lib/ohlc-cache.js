...
export async function loadOHLCCache() {
  try {
    ...
    if (!data || typeof data === "object" === false) return {};

    // Migrate any legacy ".NS"-suffixed keys to clean keys (one-time, cheap)
    const cache = {};
    for (const [key, val] of Object.entries(data)) {
      const clean = key.replace(".NS", "").toUpperCase();
      if (!cache[clean]) cache[clean] = val; // prefer clean key if both exist
    }
    return cache;
  } catch {
    return {};
...
export async function refreshOHLCCache(kite, tokenMap, symbols, cache) {
  ...
  for (const sym of symbols) {
    const key    = sym.replace(".NS", "").toUpperCase();
    const token  = tokenMap[key] ?? null;
    if (!token) { stats.failed++; continue; }

    const cached   = cache[key] || [];
    const lastDate = cached.length ? cached[0].date : null;

    // Already up to date for today
    if (lastDate === today) { stats.skipped++; continue; }

    try {
      await sleep(100); // stay under Kite historical data rate limit
...
export async function getRefreshedCache(kite, tokenMap, symbols) {
  ...
  // Fast path: if every requested symbol is already cached for today, skip all API calls
  const today = nowIST().toISOString().split("T")[0];
  const allFresh = symbols.every(s => {
    const key    = s.replace(".NS", "").toUpperCase();
    const cached = cache[key] || [];
    return cached.length > 0 && cached[0].date === today;
  });

  if (allFresh) {
    console.log(`[ohlc-cache] fully cached for today — skipping refresh`);
    return { cache, stats: { fetched: 0, skipped: symbols.length, failed: 0 } };
  }
