...
// Rate-limit helper: 350ms between historical data calls (max 3 req/sec)
 ...
 */
export async function runSellCheck({ force = false } = {}) {
  ...
  // Batch all LTP calls into one request (up to 1,000 symbols per call)
  const allSyms = [...new Set(unsold.map((r) => (r["Symbol"] || "").replace(".NS", "").trim()).filter(Boolean))];
  const ltpMap  = allSyms.length ? await getLTP(kite, allSyms) : {};
  ...
  for (const stock of unsold) {
    ...
    try {
      ...
      if (!sellReason && now.getHours() === 15 && now.getMinutes() >= 15) {
        await sleep(350); // respect 3 req/sec historical data limit
  ...
  await setCronState("sell-check", { enabled: true, lastRun: new Date().toISOString(), lastStatus: "ok", sells: sells.length });
 ...
 */
export async function runBuySignalScan({ force = false } = {}) {
  if (!isMarketOpen() && !force) return { skipped: true, reason: "Market closed" };

  const kite    = await requireKiteToken();
  const rows    = await getExcel("stocks_bought-atsl.xlsx");
  const unsold  = rows.filter((r) => !r["Sell Date"]).slice(0, 20);
  const signals = [];

  // Batch all LTP calls into one request
  const allSyms = [...new Set(unsold.map((r) => (r["Symbol"] || "").replace(".NS", "").trim()).filter(Boolean))];
  const ltpMap  = allSyms.length ? await getLTP(kite, allSyms) : {};
  ...
  return { ok: true, signals, forced: force };
}
...
export async function runATSLUpdate({ force = false } = {}) {
  ...
  // EOD force-close open positions after 15:25 — batch LTP for all unsold
  if (isCloseWindow) {
    const unsold    = rows.filter((r) => !r["Sell Date"]);
    const closeSyms = [...new Set(unsold.map((r) => (r["Symbol"] || "").replace(".NS", "").trim()).filter(Boolean))];
    const closeLtp  = closeSyms.length ? await getLTP(kite, closeSyms) : {};

    for (const stock of unsold) {
      const sym = (stock["Symbol"] || "").replace(".NS", "").trim();
      if (!sym) continue;
      const cmp = closeLtp[sym.toUpperCase()] ?? null;
      if (!cmp) continue;
      const idx = rows.findIndex((r) => r["Symbol"] === stock["Symbol"] && !r["Sell Date"]);
      if (idx !== -1) {
        rows[idx]["Sell Date"]   = todayIST();
  ...
  // Buy window — batch LTP for all signals
  if (isBuyWindow) {
    const signals  = await getJSON("buy_signals_today.json") || [];
    const WALLET   = 100000;
    const buySyms  = [...new Set(signals.slice(0, 3).map((s) => (s.symbol || "").replace(".NS", "").trim()).filter(Boolean))];
    const buyLtp   = buySyms.length ? await getLTP(kite, buySyms) : {};
    ...
    for (const signal of signals.slice(0, 3)) {
      ...
      try {
        const cmp = buyLtp[sym.toUpperCase()] ?? null;
        if (!cmp) continue;
  ...
  await setCronState("atsl-update", { enabled: true, lastRun: new Date().toISOString(), lastStatus: "ok", actions: actions.length });
...
export async function runTokenHealth() {
  ...
  if (status.valid) {
    await sendTelegram(
      `🌅 *Good Morning — Daily Token Reminder*\n\n` +
      `✅ Token is valid\nUser: ${status.user_name} (${status.user_id})\n\n` +
      `If Zerodha logged you out overnight, refresh here:\n🔗 ${loginUrl}`
    );
  } else {
    await sendTelegram(
      `🌅 *Good Morning — Token Refresh Required*\n\n` +
      `❌ Token is invalid\nReason: ${status.reason}\n\n` +
      `Please login now to enable trading:\n🔗 ${loginUrl}`
    );
  }
