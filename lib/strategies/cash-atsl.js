...
/**
 * Fetch closes directly using a pre-resolved token to avoid per-call KV lookups.
 */
async function fetchClosesByToken(kite, token, days = 30) {
  const now  = nowIST();
...
async function fetchOHLCByToken(kite, token, days = 10) {
  const now  = nowIST();
  const from = new Date(now);
  from.setDate(from.getDate() - Math.ceil(days * 1.5));
  const toStr   = now.toISOString().split("T")[0];
  ...
  const data = await kite.getHistoricalData(token, "day", fromStr, toStr, false, false);
  if (!Array.isArray(data) || data.length === 0) return [];
  return data.slice(-days).map((c) => Array.isArray(c)
...
export async function runCycle(kite, def, { now, forceBuy = false } = {}) {
  ...
  for (const stock of unsold) {
    ...
    try {
      ...
      if (!sellReason && isPostClose) {
        ...
        const closes = token ? await fetchClosesByToken(kite, token, 30).catch(() => []) : [];
  ...
  if (isBuyWindow) {
    try {
      ...
      // Load pre-computed Nifty200 stock list from R2; fall back to hardcoded list
      const nifty200 = await getJSON("EQUITY_L_NIFTY200_symbols.json").catch(() => null);
      const universe = ((nifty200 && nifty200.length) ? nifty200 : NIFTY200_SYMBOLS).filter(Boolean);
      ...
      for (const rawSym of universe) {
        ...
        try {
          await sleep(350);
          const candles = await fetchClosesByToken(kite, token, 30);
          if (candles.length < 8) { filterLog.noData.push(sym); continue; }
          ...
          const ohlcCandles = await fetchOHLCByToken(kite, token, 10);
          if (ohlcCandles.length < 2) { filterLog.noData.push(sym); continue; }
