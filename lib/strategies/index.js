...
export const STRATEGY_DEFS = [
  {
    ...
    config: {
      ...
      flatStopLossPct: 2.0,    // hard stop: sell if loss > 2%
      maxHoldDays: 7,
  ...
  },
  {
    ...
    name: "Above 6EMA",
    version: "1.0",
    description: "Buy stocks where prev close > 6EMA, today close > 6EMA, today > prev close, and CMP > 6EMA. NIFTY 50 must be above its own 6EMA. Adaptive TSL exit or EMA breakdown.",
    mode: "paper",
    status: "active",
    config: {
      universe: "nifty200",
      ...
      maxHoldDays: 10,
      walletBalance: 1000000,
      maxPositions: 50,       // up to 50 positions @ ₹10K each = ₹5L max deployed
      maxCapitalPerStock: 10000,
      maxScan: 150,           // scan more of Nifty 200
    },
  },
];
...
export async function runStrategyCycle(id, opts = {}) {
  ...
  if (!isMarketOpen() && def.mode !== "paper" && !opts.forceBuy && !opts.forceSell) {
  ...
  switch (id) {
    ...
    case "btst":
      ...
      break;
    case "ema-above":
      agent = await import("./ema-above.js");
      break;
    default:
