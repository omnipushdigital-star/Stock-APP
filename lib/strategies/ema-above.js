...
export async function runCycle(kite, def, { now, forceBuy = false, forceSell = false } = {}) {
  ...
  // Probe one symbol to detect why getDailyCloses might be failing
  if (candidates.length) {
    try {
      const { getToken } = await import("../instruments.js");
      const tok = await getToken(kite, candidates[0]);
      if (!tok) {
        diag.closesError = `No instrument token for ${candidates[0]} — instruments map may be empty in KV`;
      } else {
        const probe = await getDailyCloses(kite, candidates[0], 15);
        if (probe.length === 0) diag.closesError = `getHistoricalData returned empty for ${candidates[0]} (token: ${tok})`;
        else diag.closesError = null; // working fine
      }
    } catch (e) {
